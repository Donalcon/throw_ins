...
def prediction_feature_engineering(df, stats_cols, elo_ratings):
    ...
    df, _ = calculate_elo(latest_results, df)
    # Fill NaN values in 'team_elo' based on the team's latest Elo rating
    df['team_elo'] = df.apply(
        lambda row: elo_ratings.get(row['team'], row['team_elo']) if pd.isna(row['team_elo']) else row['team_elo'], axis=1
    )
    # Fill NaN values in 'opp_elo' based on the opponent's latest Elo rating
    df['opp_elo'] = df.apply(
        lambda row: elo_ratings.get(row['opp'], row['opp_elo']) if pd.isna(row['opp_elo']) else row['opp_elo'], axis=1
    )
